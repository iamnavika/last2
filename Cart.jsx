...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    const orderInfo = { cartItems, addressInfo, date: new Date().toLocaleString(), email: JSON.parse(localStorage.getItem("user")).user.email, userid: JSON.parse(localStorage.getItem("user")).user.uid }

    // Razorpay payment integration here...
      ...
      <div className="h-screen bg-gray-100 pt-5 mb-[60%]" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '' }}>
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
                ...
                <div className="justify-between mb-6 rounded-lg border drop-shadow-xl bg-white p-6 sm:flex sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '' }}>
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    ...
                    <div className="mt-4 sm:mt-0 sm:flex sm:space-x-4">
                      ...
                      <EmailShareButton url={shareLink} subject={`Check out this product: ${title}`}>
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '' }}>
            ...
            <div className="flex justify-between">
              <p className="text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>Discount</p>
              <p className="text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>₹-{Discount}</p>
            </div>
            <hr className="my-4" />
